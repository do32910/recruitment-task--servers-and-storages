...
const Storages = () => {
  ...
  function search(searchTerm: string): void {
    if(!searchTerm) {
      ...
      const matchingServers = servers.map(server => {
        server.displayed = (server.hostname.includes(searchTerm) || server.title.includes(searchTerm));
        return server;
      })  
  ...
  function generateCardContent(): JSX.Element | JSX.Element[] {
    ...
    return !!servers.length && servers.map(server => (
        server.displayed && 
